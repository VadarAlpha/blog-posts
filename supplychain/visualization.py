...
def assign_supplier_types(locations, supplier_types):
    # Shuffle the supplier types list
    random.shuffle(supplier_types)
    # Create a cycle iterator from the shuffled list
    supplier_types_cycle = itertools.cycle(supplier_types)
    return {
        location: {
            "coordinates": coords,
            "supplier_type": (
                ...
                else next(supplier_types_cycle)
            ),
        }
...
def visualize_supply_chain(
    ...
    for location, details in locations_with_suppliers.items():
        ...
        start_node = ox.distance.nearest_nodes(
            G, X=start_location[1], Y=start_location[0]
        )
        end_node = ox.distance.nearest_nodes(
            G, X=wausau_location[1], Y=wausau_location[0]
        )
        ...
        try:
            ...
            logging.info(
                f"Shortest path for {location} to Wausau computed successfully"
            )
            ...
            folium.PolyLine(
                locations=path_coords, color=color, weight=4, opacity=0.7
            ).add_to(m)
